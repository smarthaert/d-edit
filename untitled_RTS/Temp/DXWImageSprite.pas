...
destructor TWImageSprite.Destroy;
var
 Index,i : integer;
begin
// remove from the selected list & group lists
with SelectedUnitList do
  begin
  remove(self);
  for i := 0 to MaxGroups -1  do
    begin
    Index := UnitGroups[i].indexof(self);
    if Index <> -1 then
      UnitGroups[i].Delete(Index);
    end;
  end;
inherited Destroy;
end; {Destroy}
