...
begin
  ...
  if MemoryLimit>0 then begin
    ...
    if not processmemorylimitdata (ProcessID, CurMemoryUsed) then
     // LogString ('Unable to determine memory usage properties for PID=%u', [ProcessID]) Denis
    else begin
...
begin
  LogString ('Фактическое время выполнения: %u ms', [GetTickDist (starttick)]);
end;


procedure LogMemoryUsage (MaxMemoryUsed:int64);
begin
  LogString ('Использовано памяти: %u Kb', [MaxMemoryUsed div 1024]);
end;
