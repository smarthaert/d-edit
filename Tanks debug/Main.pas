...
type
  ...
  public
    ...
    BaseDead,ShowInfo,GameInitialized:Boolean;
    GameMenu:TMyMenu;
    KeyDelay:LongInt;
...
begin
  ...
  BeforeGameScreen:=TBeforeGameScreen.Create(Self);
  MapContainer:=TMapContainer.Create(Self,MapImageList);
  For I:=1 to 2 do
    Begin
      PlayerTanks[I]:=Nil;
      Lives[I]:=0;
  ...
  {}
  WeaponTypes:=TWeaponTypes.Create(Self);
  ...
  TankTypes:=TTankTypes.Create(Self);
  ...
  {GameMenu}
  GameMenu:=TMyMenu.MyCreate(Self,320-100,100);
  With GameMenu do
    Begin
...
begin
          ...
          Begin
            ...
            For J:=1 to 2 do
              If (Lives[J]>0)And(PlayerTanks[J]=Nil) then
                If HandleTimeDec(LivesDelay[J],20) then
                  TryPlayerSpawn(J);
            DXSpriteEngine.Move(20);
            DXSpriteEngine.Dead;
          End;
    End;
  ...
  {  Description  }
  MapContainer.PaintMap(DXDraw.Surface);
