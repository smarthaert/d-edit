...
CGraph::CGraph(int Tops, int Edges, int Style)
{
	...
	while (Edges != m_Edges)
	{
		...
		a=Random(1,m_Tops);
		b=Random(1,m_Tops);
		while (a==b || IsLinked(a,b) || IsLinked(b,a) || (IsPathPresent(a,b) && !(Style & Gr_Cycled)))
		{ 
			a=Random(1,m_Tops);
			b=Random(1,m_Tops); 
		} 
...
void CGraph::SetEdge(int i, int j)
{
	...
	if (m_Style & Gr_Oriented)
	{
		if (m_Style & Gr_Price)
		{
			...
			{
				m_Cost[i*m_Tops+j] = Random(1,MAXCOST);
				m_Adjective[i*m_Tops+j] = 1;
			}
			else
			{
				m_Cost[i*m_Tops+j] = Random(-MAXCOST/2,MAXCOST/2);
				m_Adjective[i*m_Tops+j] = 1;
			}
		}
		else
	...
	else
	{
		if (m_Style & Gr_Price)
		{
			if (m_Style & Gr_Positive)
			{
				m_Cost[j*m_Tops+i]=m_Cost[i*m_Tops+j] = Random(1,MAXCOST);
				m_Adjective[i*m_Tops+j] = 1;
				m_Adjective[j*m_Tops+i] = 1;
			}
			else
			{
				m_Cost[j*m_Tops+i]=m_Cost[i*m_Tops+j] = Random(-MAXCOST/2,MAXCOST/2);
				m_Adjective[i*m_Tops+j] = 1;
				m_Adjective[j*m_Tops+i] = 1;
			}
		}
...
{
	return m_Adjective[(i-1)*m_Tops+j-1] !=0;
}
...
int CGraph::GetPrice(int i, int j)
{
	return m_Cost[(i-1)*m_Tops+j-1];
}
